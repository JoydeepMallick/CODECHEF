...
void test(){
  ...
  ll nearestprime = h;
  ll minsteps = LONG_MAX;
  bool found =  false;
  while(nearestprime >= 2){
    if(!isprime[nearestprime]){
      nearestprime--;
      continue;
    }

    ll remain = h - nearestprime;
    ld logval = log2(remain);

    dbg(nearestprime, remain, logval);
    if(floorl(logval+1) == ceill(logval+1)){
      found = true;
      ll steps = (1 + floorl(log2(logval+1)));
      minsteps = min(minsteps, steps ) ;
      break;
    }

    nearestprime = 1 + nearestprime/2;
  }
  
  if(found) cout << minsteps << endl;
  else cout << "-1\n";
