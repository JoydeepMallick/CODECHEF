...
void test(){
    ...
    /*
     * the 2nd operation of spliting is actually skipping his/her turn by simple splitting if possible 
     *
     * now if sum of all the array elements is odd then Chef wins only if first removal happens from Cheffina
     * so both will try splitting intially until all are one.
     *
     * if sum is odd, chef will try to remove elements starting first so that he wins 
     * assume  2  1   then chef removes 1 from 2 (>1)  = 1 1 then  cheffina then chef so CHEF wins  
     * assume 3  2    
     */
