...
void test(){
    ...
    cin >> x >> y;
    int win = 0;
    //each will try to reduce opponent pile  to 0 so that no moves can be performed an both ways at end then can safely reduce the leftover pile of his/her own by 2 
    /*
     * both odd and (sure that diff is always at least 2)
     *      if x > y = CHEF  else x < y = CHEFINA
     *      if x == y CHEF
     *
     *both even and  
            if x > y = CHEF else x < y = CHEFINA
    ...
    else if one odd and one even 
            if(x>y and  x is odd) assume 7 6  and  9 6   then if (x-y) > 1 then CHEF else CHEFINA
            if(x<y and x is even) then CHEFINA
            if(x>y and x is even)  then CHEF
            if(x<y and x is odd) then assume 5,6    and 5,8   if(y-x) > 1 CHEFINA else CHEF
     */
