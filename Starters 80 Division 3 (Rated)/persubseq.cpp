...
void test(){
    ull n;
    cin >> n;
    ull a[n+1] = {0};//since we are concerned with a subsequence frequency of elements within range <= n must only be considered
    fori(0,n){
        ull ele;
        cin >> ele;
        if(ele <= n) a[ele]++;
...
/* ull prefixmul[n+1] = {0}; */
    // if 1->2   2 ->3  3-> 0 ....
    // then in prefixmul  1 -> 2  , 2 -> 2*3, .....
    //prefixmul[0] = 1;
    ull previousmul = 1;
    ull ans = 0;
    fori(1, n+1){
        if(a[i]){
            //prefixmul[i] = (prefixmul[i-1] * a[i]) % mod;
            ans = (ans + (previousmul*a[i]) % mod) % mod;
            previousmul = (previousmul*a[i]) % mod;
        }
        else break;//the moment first 0 encountered break away
    }
    cout << ans << endl;
}
