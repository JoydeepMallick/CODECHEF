...
void test(){
    int n,m,k;
    cin >> n >> m >> k;
    ull temp = modmul(n,m,mod);
    ull ans = (modmul(temp, temp+1,mod)/2) % mod;
    fori(0,k){
        int q,x,v;
        cin >> q >> x >> v;
        if(q==0){
            temp = (modmul(x-1,m,mod) + modmul(m, m+1, mod)/2) % mod;
            dbg(temp);
            temp = modmul(temp, v-1, mod);
            dbg(temp);
            ans = (ans + temp) % mod;
        }
        else{
            temp = modmul(v, n, mod);
            dbg(temp);
            temp = modmul(temp, (x + (modmul(n-1,m,mod))/2) % mod, mod);
            dbg(temp);
            ans = (ans + temp) % mod;
        }
    }
