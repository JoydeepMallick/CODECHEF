...
void test(){
    ...
    sort(combinedab, combinedab + 2*n);
    //we are sure that differnce is positive since array elements are always grter than equal 1 according to constraints
    //smallest differnce can only be attained when max and min are as close to each other. this can be guranteed if we check for for diff each time in combinedab using sliding window technique
    int diff = INT_MAX;
    fori(0,n+1){
        diff = min(diff, combinedab[i + n-1] - combinedab[i]);
    }
    cout << diff << endl;
}
