...
void test(){
    ...
    cin >> n;
    /*
     Among 3 numbers we can assume 1 as a number which can divide all numbers hence we are left with other 2 distinct numbers b and c

     now we can definitely say any prime number will definitely return -1 since it cant have more than 2 factors and we need 3 numbers at least.

     Now we can iterate from 2 till root of n and find two factors wherein product of 2 DISTINCT factors will only result in a answer to exist.

     if a number is a perfect square answer will exist except for 1.
     */
    ll b = -1, c = -1;
    for(ll i = 2; i*i < n; i++){
        if(n%i==0){
