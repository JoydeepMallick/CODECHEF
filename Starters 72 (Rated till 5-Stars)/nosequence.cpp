...
void test(){
    ...
    cin >> n >> k >> s;
    /*
     * series sum = s = b1*k^0 + b2*k^1 + b3*k^2+.....+ bn*k^(n-1)
     *
     * here we notive that b1 is the only free term, possible alues -1, 0 , 1
     *
     * hence we can defintely check if s is divisible by k then its safe to say b1 is 0 else remainder if 1 then 1 else for any other value -1
     * 
     * now since we managed b1 (s-b1)/k1 must be equal to b2*k^0 + b3*k^1+.....+ bn*k^(n-2)
     *
     * such will continue and we subsequently discover each b.
     *
     * at las twe recheeck if sum of all the b and k raised to sutiable power eqals s or note
     * if yes print the values of b else print -2
     *

     */
    vector<int> b(n, 0);//set all vlues intially to 0
    fori(0,n){
      if(s%k==1){
        s--;//s+bi = s+1
        b[i] = 1;
      }
      else if(s%k == (k-1)){//this can be verified
        s++;// its actually s -(- bi) since bi is a -1 = -bi
        b[i] = -1;
      }
      else if(s%k){
        //any other value but ot 0
        s = 1;//not 0
        break;
      }
      s/=k;
    } 
    if(s){//if s doesn't turn out to be 0
      cout << -2 << endl;
    }
    else{
      fori(0,n) cout << b[i] << " ";
