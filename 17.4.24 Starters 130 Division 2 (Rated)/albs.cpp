...
void test() {
  ...
  /* vll conseq; // 00000 1 0 111 0 1 0 1111   count each such continuous sets
   */
  /* conseq.pb(1); */
  /* fori(1, n) { */
  /* if (s[i] == s[i - 1]) { */
  /* conseq[conseq.size() - 1]++; */
  /* } else */
  /* conseq.pb(1); */
  /* } */
  /* dbg(conseq); */
  ...
  vll cnt; // 111 1010 00 010101 101 count sets with alternates and equals
  bool gap = false;
  fori(0, n - 1) {
    if (s[i] == s[i + 1]) {
      if (cnt.size() == 0 || gap == true) {
        cnt.pb(1);
        gap = false;
      } else {
        cnt[cnt.size() - 1]++;
