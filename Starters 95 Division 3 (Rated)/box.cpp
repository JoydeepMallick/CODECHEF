...
/*
 //brute force gives TLE
    ...
    for(auto ele : a){
        if(boxes.size() == 0) boxes.pb(ele);
        else{
            bool oldboxused = false;
            fori(0, boxes.size()){
                if(boxes[i] >= ele){
                    boxes[i] ^= ele;
                    oldboxused = true;
...
*/
    /*
Observations :-
1. we can drop all the elements that dont have the MSB as set into the same box where only one element with MSB set will be allowed so that all elements entering the box will be smaller than the xor of all elements already present in the box since MSB will always be 1 in XORed number of box.e.g. [1101, 0110,0101,0111]

2.  For the rest of the elements with elements as set they can be grouped into pairs.    eg. [1101,  1001]
     
    * Hence first find the element with highest MSB i.e. the max element in array
    * then find the MSB.
    * for ele in array :
    *   if MSB of ele == MSB : cnt++
    this cnt essentially counts the number of elements in array with MSB set.

    Now out answer will be all paired up except for 1 which will store other off MSB numbers.
    ans  = (cnt+1)/2;

*/
    
    ll mxele = *max_element(a.begin(), a.end());
    int MSB_mx = log2(mxele);
    int cnt = 0;
    fori(0,n){
        dbg(a[i], log2(a[i]));
        if((int)log2(a[i]) == MSB_mx) cnt++;
    }
    dbg(mxele, MSB_mx, cnt);
    cout << (cnt+1)/2 << endl;
}
