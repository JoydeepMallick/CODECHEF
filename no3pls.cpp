...
void test(){
    ...
    //post rotations for type 1 
    // 1 2 1 1 2 1 2 1 2 rotated at index 3
    // 2 1 2 1 1 1 2 1 2 rotated at index 4
    // hence once 3/2 ones must exist and rest times only 1 ones between single 2 
    // adjacent cnt for first one 1 2 1 1 2 1 2 1 2 
    //                            1 1  2  1 1 1 1 1 
    //
    // post rotatiosn of type 2 
    // 2 1 2 1 2 2 1 2 rotated at index 4
    // 1 2 1 2 2 2 1 2  rotated at index 3
    // hence post rotation once 3/2 twos must exit and rest times only one 2 between 1 
    //adjacent count for second  example 1 2 1 2 2 2 1 2 
    //                                   1 1 1   3   1 1 
    ...
    adjcnt.push_back(1);
    fori(1,n){
      if(a[i-1] == a[i]) adjcnt.back()++;
      else adjcnt.push_back(1);
    }
  ...
  //best case actual order, then adjcent cnt follows 
  // 2 1 1 1 1 ... 
  //
  // Edge case when seq is 1 1 or 2 2 
  // then its 2 
  //
  // Assume valid rotations of 1 1 2 1 2 1 2 
  //                               ^ ^ ^ ^ 
  //                               a b c d 
  //  a. 2 1 1 1 2 1 2 
  //     1   3   1 1 1  <= cnt 
  ...
  //     1 1 1 1  2  1 <= cnt 
  //
  //     Similar pattern follows for other 
  //
  //     Observations
  //
  //     1 ... (2x times where x >= 0)  2  1 1 1 .....(0 or more times)
  //     1 ... (3x times where x > 0 not 0) 1 1 1 ....(0 or more times)

    
    dbg(adjcnt, cnt[1], cnt[2], cnt[3]);
    if(adjcnt.size() == 1 && adjcnt[0] < 3) yes;

    else if(cnt[1] != adjcnt.size()-1 || cnt[2] + cnt[3] > 1 || cnt[2]+cnt[3] == 0 ){
      no;
      dbg("2nd if");
    } 
