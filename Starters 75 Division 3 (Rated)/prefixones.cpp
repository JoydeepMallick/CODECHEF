...
void test(){
    ...
    cin >> n >> s;
    //2 cases
    //1. when s[0] is 1 just print length of max consequtive ones
    //2. else find the length of longest of consecutive ones which make longest prefix and the first set starting from ones 
    //          i.e.   110010111011011110
    //                 2   1  3   2  4 
    //                 2     +       4   is longest 
    // exception :-
    //
    //                 0000000000000 all zero
    //                 answer is 0
    //                 111..0000000000000000..
    //                 answer is the first count of 1s
    ...
    fori(0,n){
        if(s[i]=='1'){
            ...
            onecounts.push_back(c);
            c = 0;
        }
    }
    if(c > 0) onecounts.push_back(c);

    if(onecounts.size() < 2){
        if(onecounts.size() == 0) cout << 0 << endl;
        else cout << onecounts[0] << endl;
    }
    else if(s[0] == '0'){
        cout << *max_element(onecounts.begin(), onecounts.end()) << endl;
    }
    else{
        //from 2 element till last find max
        cout << onecounts[0] + *max_element(onecounts.begin()+1, onecounts.end()) << endl;
