...
void test(){
   ...
   vi setbits;
   fori(0,n){
       if(bin[i]=='1') setbits.push_back(n-i-1);
   }
   //above will be in decending order
/* S->bin will NOT contain leading zeros. */

   //a number less than 3 can never be represented as 3 powers of 2 sum as 1 + 1 + 1 is the least possible.
   //if 3 bits are set in the number then it will surely be sum of 3...
   //but more than 3 bits bits if set never possible as we can break 2^ k to 2^(k-1) + 2^(k-1) but 2 ^k and 2 ^m can never be represented as a combined sum power of 2 by any means until 
// k = m
       // 1, 01, 10, 101011,  not allowed
       // 100, 11, 101, 100001000, 100001,........... all possible

   if((n==2 && bin[0]=='1' && bin[1]=='1') || (n > 2 && setbits.size() <= 3)) yes;
   else no;
}
