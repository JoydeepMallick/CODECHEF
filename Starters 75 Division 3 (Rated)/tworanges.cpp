...
void test(){
    int a,b,c,d;
    cin >> a >> b >> c >> d;
    //two cases when two ranges overlap else both ranges seprate
    if((b >= c && b <=d) //when first range lies before 2nd
            ||  
            (d>=a && d <=b)// when 1st range is after second
      )
    {
        cout << max(a,max(b, max(c,d))) - min(a,min(b,min(c,d))) + 1 << endl;

    }
    else{//distinct seprate ranges
        cout << abs(a-b)+1 +abs(c-d)+1 << endl;
    }
}
