...
void test(){
    ...
    //remove the pairs of same elements as they can can be changed to desired number 
    //then check if remaining all elements are same or not in similar fashion till no such is possible , then if strijng length is still 1 or more not possible.
   if(check(s,n)) yes;
   else no;
}
