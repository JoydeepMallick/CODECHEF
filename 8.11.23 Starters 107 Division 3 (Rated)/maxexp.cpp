...
/* You can choose X from 0 to min(N,K-1). */

/* Reason: */

/* We always mod K both the terms hence the highest value of (X mod K) and ((N-X) mod K) can be K-1 */
/* Also we need to choose X between 0 and N */

/* Now for N=4 and K=7, */

/* (X mod K) can be [0,1,2,3,4] (i.e) X value */

/* ((N-X) mod K) will be the rotated version of this based on N value. Here it is [4,3,2,1,0] */

/* Now we need to find the maximum value of multiplication from the above 2 list of values and print it's corresponding X value. */


//w.k.t X is always between 0 <= X <= min(N,K-1)

/* Now if N=8 and K=6 */

/* X can be [0,1,2,3,4,5] */

/* ((N-X) mod K) will be [2,1,0,5,4,3] */

/* Here when X=4, we get 4 X 4 = 16 (max) hence we choose this X. For this the formula */
