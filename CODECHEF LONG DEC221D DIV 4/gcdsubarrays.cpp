...
void test(){
    int n,k;
    cin >> n >> k;
    //remember gcd of negative numbers equals positive numbers, hence we are
    //concerned with only absolute values hence only positive numbers needed
    //moreover only positive integers given
    //hence no 0 allowed
        //we can surely say that the sum must be grater than or equal first
        //n natural numbers
        //e.g. [1 1 1 1 1.........] , 
        //sum = 1+1+1+.....+ all possible grouping with 1
    int s = n*(n+1)/2;
    if(k < s){
      cout << -1 << endl;
      return;
