...
void test(){
	string a,b;
	cin >> a >> b;
	//dissimilar elements must have same higher index
	//at last similar elements must be placed to keep 0 as small as possible
	...
	int no_of_1 = min(a0,b1) + min(b0,a1);
	int no_of_0 = min(a0+a1, b0+b1) - no_of_1;
	no_of_1 += abs((a0+a1) - (b0+b1));//if a and b of unequal length then the abs
								 //diff is adding extra 1s to answer
	fori(0, no_of_1) cout << 1;
	fori(0, no_of_0) cout << 0;
	cout << endl;
}
