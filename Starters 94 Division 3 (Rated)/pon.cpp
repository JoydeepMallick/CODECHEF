...
void test(){
    ...
    for(auto ele : a){
        ll op = ele&b;
        if(op==b) and_a.pb(ele);//store all elements with capability to make 5.i.e. have set bits of 5
    }
    dbg(and_a);
    
    //if and of all selected numbers is equal to b then possible else not 
    int ans = -1;// all bits are 1 
    for(auto ele : and_a){
        ans &= ele;
    }
    if(ans==b) yes;
