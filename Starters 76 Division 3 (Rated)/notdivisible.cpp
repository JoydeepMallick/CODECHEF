...
void test(){
  ...
  cin >> n;
    /*
     * a number less than n is never divisible by n 
     *
     * now j - i >= 1 as j > i and i min is 1
     * hence j -i+1 >= 2 hence any two alternate elements sum if less than 2 then its not divisible
     *
     * assume j = 3 i = 1 then j-i+1 >= 3 so any three continuous no sum if less than 3 then number is not divisible
     *
     * ASSUME A sequence to be 0 1 0 1 0 1.....
     * or 1 0 1 0 1......here all conditons are satisfied
     */
  fori(0, n){
     if(i&1) cout << 0 << " ";
     else cout << -1 << " ";
