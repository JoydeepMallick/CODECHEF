...
void test(){
    ...
    /*
     * observation
     * xor of odd and even element is always odd hence our target is xor two elements of different parity
     *
     * or of a odd element with another is always odd
     *
     * hence if array contains all even elements then its only possible if x is odd.
     */
    if(odd == n) cout << 0 << endl;
    else if(even == n){
        if(x&1) cout << n/2 + n%2 << endl;
        else cout << -1 << endl;
    }
    else{
        //both odd and even elements present
        //2 cases if x odd and even
        if(x&1){
