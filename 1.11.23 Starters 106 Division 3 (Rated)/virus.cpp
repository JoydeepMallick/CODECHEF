...
void test(){
    ...
    /*
    at max 4 options to place antivirus(vaccine used interchangeably) around the disease.
             +
        +    A   +
    +   A  virus  A  +
        +    A    +      
             ...
             + denotes spaces where we can spread at next momement at best we can spread antivrus in 3 spaces at a time, out task is to find such a position among 4 allowed positions around disease which has max action.

             If we follow this optimal method we will always attain two rectangles one of diease and one of antivirus.


             We need to find the point which will give us the biggest rectangle for vaccinated portion.
             here its 1-based indexinf note this and also indexing starts from top like:-
                  m
             ------------>
        |    1,1   1,2  1,3  .......
     n  |    2,1   2,2  2,3  .......
        ...
        v    .......................
             ...
             If vaccine on top,  maxarea covered = (x-1) * m for upper rectangle
             If vaccine on left, maxarea covered = (y-1) * n for left rectangle
             If vaccine on right, maxarea covered = (m - y) * n for right rectangle 
             If vaccine on down, maxarea covered = (n - x) * m for down rectangle 

             now the max area is the answer.
*/
    
    ll maxarea = max({(x-1)*m,  (y-1)*n,  (m-y)*n,  (n-x)*m  });
    cout << maxarea << endl;
}
