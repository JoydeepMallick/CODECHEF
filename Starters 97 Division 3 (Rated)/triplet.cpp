...
void test(){
    ...
    fori(0,n){
        pref[i] = (n-i-1)*(n-i-2)/2;// smallest elemtent repeated nC2 times , then 2nd smallest (n-1)C2 times and so on 
    }
    ...
    fori(0,q){
        ll k;
        cin >> k;
        ll ind = lower_bound(pref.begin(), pref.end(), k) - pref.begin();//returns ind of element smaller than or equal to k
                                                                        cout << a[ind] << endl;
    }
}
