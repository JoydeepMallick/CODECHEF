...
void test(){
  ...
  //edge case is when n = 4 then if a[0] + a[1] == a[2] + a[3] then not possible 
  //else for n > 4 if any one element in the entire array is different then it is always posssible
  if(n == 4){
    if((a[0] + a[1]) == (a[2] + a[3])) no;
    else yes;
  }
  else{
    sort(all(a));
