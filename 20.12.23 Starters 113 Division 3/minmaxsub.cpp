...
void test(){
  ll x,y;
  cin >> x >> y;
  //all clusters of 1 must of equal size expect the last 1 at most
  //now either sum of cluster will be maximum or the entire array sum will be maximum 
  //
  //1 1 1 -2 1 1 1 -2 1 1     max cluster sum = 3 , total sum = 4  hence ans = 4
  //1 1 -2 1 1 -2 1 1 -2      max cluster sum = 2 and total sum = 0 hence ans = 2 
   ll cluster = ceill(x/(y + 1.0)); 
  cout << max(cluster,  x - 2*y) << endl;
