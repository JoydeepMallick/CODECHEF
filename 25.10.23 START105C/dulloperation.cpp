...
std::vector<int> getMinProductNumbers(int n) {
    ...
    std::vector<int> minNumbers = {minA, minB};
    return minNumbers;
}
...
void test(){
    ll n;
    cin >> n;
    //after dry run possible x and y are as minimum apart as possible 
    vector<int> factors  = getMinProductNumbers(n);
    
    dbg(factors);
    ll y = 0;
    for (ll i = 0; i < 31; i++) {
        ll bit_a = (factors[0] >> i) & 1;
        ll bit_b = (factors[1] >> i) & 1;
