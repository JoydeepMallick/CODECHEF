...
/* array<int, n> a{}; */
  ...
  /*
   * marickha tries to remove max stone piles at once privided they have a odd frequency
   *
   * zenska has the oppostire target of removing max with even frequency
   *
   * observation:-
   *
   * marichka can only win when at least freq of any one element is odd
   * else if freq if each pile is even zenyk wins
   */
  
  dbg(freq);
  bool oddpileexists = false;
  for(auto it : freq){
    if(freq[it.first] & 1){
      oddpileexists = true;
      break;
    ...
    if(oddpileexists) cout << "Marichka\n";
    else cout << "Zenyk\n";
}
