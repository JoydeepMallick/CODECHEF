...
void test(){
  ll x;
  cin >> x;
  int msb = floorl(log2(x));
  ll b = 1 << msb;
  ll a = (x^b);

  // 10110 =  1 0 0 0 0  
  //          0 0 1 1 0 
  //                  -   bits can be altered as wished but together(a,b < x will hold) 
//  for 101010    =  1 0 0 0 0 0
//                       1 0 1 0  
//                         -   - 
//Hence total alteration count will be 2 power cnt 
  //exceptrion 
  if(x == b) {
    cout << "1\n";
    return;
  ...
  ll cnt = 0;
  ll msb_a = floorl(log2(a));
  fori(0, msb_a+1){
    ll mask = 1 << i;
    ll vala = a & mask;
    ll valb = b & mask;
    dbg(x,a,b,i,vala, valb);
    if(vala == valb && valb == 0) cnt++;
  }
  cout << powl(2,cnt) << endl;
