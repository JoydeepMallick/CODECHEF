...
void test(){
    ...
    //in row 1 each cell except 1,1 costs 1 each 
    // in row 2 each cell except 2,2(cost 1) and last possible cell(for even only)(cost 1) cost 2 each 
    if(n<=2) cout << 0 <<endl;
    else{
        ll cost = (n-1)/2 + (n/2)*2 - ((n&1)?1:2);
        cout << cost << endl;
    }
