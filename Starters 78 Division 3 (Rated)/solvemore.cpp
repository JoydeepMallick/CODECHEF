...
/* array<int, n> a{}; */
  int a[n], b[n];
  multiset<pair<int, pair<int,int>>> sum_time;//sorted by sum 2nd priority sum solve time
  multiset<pair<int, pair<int, int>>> sum_time_bymin;//sorted by min directly
  vector<int> sumtime_total(n);
  vector<int> sumtime_total_bymin(n);
  fori(0,n){
    cin >> a[i];
  }

  //one case still remains for 1st test case if k = 9 and 3 4 5
  //                                                      2 4 2   
  fori(0,n){
      cin >> b[i];
      sum_time.insert({a[i]+b[i], {a[i],b[i]}});
      sum_time_bymin.insert({min(a[i],b[i]), {a[i], b[i]}});
  }
    
    dbg(sum_time);
    dbg(sum_time_bymin);

    auto it = sum_time.begin();
    auto it2 = sum_time_bymin.begin();
    sumtime_total[0] = it->second.first;
    sumtime_total_bymin[0] =it2->second.first;
    ...
    it++;
    it2++;
    while(it != sum_time.end() && it2 != sum_time_bymin.end()){
        sumtime_total[i] = sumtime_total[i-1] + (previt)->second.second + it->second.first;
        sumtime_total_bymin[i] = sumtime_total_bymin[i-1] + (previt2)->second.second + it2->second.first;
        previt = it;
        previt2 = it2;
        it++;
        it2++;
    ...
    dbg(sumtime_total);
    dbg(sumtime_total_bymin);
    int ans1, ans2;
    forj(0,n){
        if(k >= sumtime_total[j]) continue;
        else {
            ans1 = j;
            break;
        }
    ...
    forj(0,n){
        if(k >= sumtime_total_bymin[j]) continue;
        else{
            ans2 = j;
            break;
        }
