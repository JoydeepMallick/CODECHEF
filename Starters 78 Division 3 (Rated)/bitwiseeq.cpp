...
void test(){
  ...
  cin >> n;
  /*
   4 properties of xor needed and the following property:-
     0 ^ n = n

     upon trial and error method and observation we can find that 0 ^ d = d = n i.e.  ((a&b)|c) = 0
     so we need to set a,b,c carefully such that d = n then we can easily compute our answer.
     ...
     assume 7    4    2   6
           ...
           7&4 =  100 
           '' | 2  = 110 
           now for n to be 7, 6 ^ d = 7  




           cant understand the logic till now🤨
   */
  if(n&1){
      //if odd 
      cout << n << " " << n+3  << " " << n+2 << " " << n+1 << endl;    
  }
  else{
      cout << n << " " << n+2 << " " << n+1 << " " << n-1 << endl;
  }
}
