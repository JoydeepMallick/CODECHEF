...
void test(){
  ...
  vector<int> a(n+1);
  fori(1,n+1)cin >> a[i];
  /*simple two pointer apporach
   *
   *
   removed list ontains {n,n-1,n-2,.....,n-k}
   finally list {1,2,3,4,....k}


   to speed up use index based apporach by storing indices of values and then performing operations 
   **/
  dbg(a);

  vector<int> ind(n+1);
  fori(1,n+1) ind[a[i]] = i;
  dbg(ind);
  int ans = 0,max_ind = INT_MIN, min_ind = INT_MAX;
  for(int ele = n; ele > 1; ele--){
    if(ind[ele] > max_ind) max_ind = ind[ele];
    if(ind[ele] < min_ind) min_ind = ind[ele];

    if(max_ind - min_ind + 1 == n-ele+1){
      //guarantee that this range contains n-ele+1 max elements from starting from n and decreasing
      ans = max(ans, max_ind-min_ind + 1);
    }
    dbg(ele, ind[ele], max_ind, min_ind, ans);
  }
  ...
  while(l < r){
    ...
    maxrange++;
    dbg(l,r,maxi, maxrange);
    if(maxrange == maxi) break;
  }
