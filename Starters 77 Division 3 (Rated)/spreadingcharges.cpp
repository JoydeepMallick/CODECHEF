...
void test(){
  ...
  cin >> n;
    string s;
    cin >> s;
    //if all 0 then print n else leading and trailing 0 discarded
    //only middle 0 such that +00000...-   or -000000....+ are accepted as 1 each provided count of zeros is odd else again 0
    int zerosinline = 0;
    int zeros = 0;
    char sym =' ';
    for(char ch : s){
        if(ch=='0') zerosinline++;
            ...
            if(sym!=' ' && ch!= sym && zerosinline&1) zeros++;
            sym = ch;
            zerosinline = 0;//reset count
        }
    }
    if(zerosinline == n) cout << n << endl;
