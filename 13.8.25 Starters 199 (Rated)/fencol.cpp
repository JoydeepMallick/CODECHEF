...
void test(){
    ...
    // find max freq element as it costs 1 minute rest take 1 minute each 
    ...
    for(auto it : freq){
        if(it.second == max_freq && !foundonce){
            ans += 1;
            foundonce = true;
        }else{
            ans += it.second;
    ...
    ll anotherans = 0;
    // if 1s exist assume single single colotring if it leads some better answer 
    for(auto it : freq){
      if(it.first == 1) continue;
      anotherans += it.second;
    }

    cout << min(ans, anotherans) << endl;
