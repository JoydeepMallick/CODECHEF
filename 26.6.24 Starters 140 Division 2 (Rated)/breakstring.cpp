...
// released post 2020 support avx2, unfortunately my Pentium 2020M doesn't
using namespace std;
...
} // LCM OF TWO NUMBERS using inbuilt function __gcd(x,y)

// This is a template function that takes in two arguments, a and b, and updates
// the value of a to be the minimum of the two. It returns true if a was
...
void __print(const multiset<T> &ms) {
  cerr << '{';
...
void __print(const unordered_set<T> &us) {
  cerr << '{';
...
void __print(const unordered_multiset<T> &ums) {
  cerr << '{';
...
template <typename T, typename... V> void _do(T t, V... v) {
  __print(t);
  if (sizeof...(v))
    cerr << ", ";
  _do(v...);
...
#ifndef ONLINE_JUDGE
#define dbg(...)                                                               \
  cerr << __func__ << "(),line " << __LINE__ << ": " << #__VA_ARGS__ << " = "; \
  _do(__VA_ARGS__);
#else
...
int check(string s, int b1, int b2) {
  if (b1 == -1)
    b1 = s.size();
  if (b2 == -1)
    b2 = s.size();
  string p = s.substr(0, b1), q = s.substr(b1, (b2 - b1)),
         r = s.substr(b2, (s.size() - b2));
  dbg(b1, b2, p, q, r);
  if (p + q + r == s && p + r == q)
    return 1;
  else
...
int f(string s, int ind, int b1, int b2, int partcnt,
      vector<vector<vector<int>>> &dp) {
  if (ind == s.size() || partcnt == 2) {
    return check(s, b1, b2);
  } else if (dp[ind][b1 + 1][b2 + 1] != -1) {
    return dp[ind][b1 + 1][b2 + 1];
  } else if (partcnt == 0) {
    // take it or not take it
    return dp[ind][b1 + 1][b2 + 1] = f(s, ind + 1, ind, b2, partcnt + 1, dp) +
                                     f(s, ind + 1, b1, b2, partcnt, dp);

  } else {
    // take it or not take it
    return dp[ind][b1 + 1][b2 + 1] = f(s, ind + 1, b1, ind, partcnt + 1, dp) +
                                     f(s, ind + 1, b1, b2, partcnt, dp);
  }
}
...
void test() {
  ...
  /* int dp[300005][300005][300005] = {-1}; */
  cout << f(s, 0, -1, -1, 0, dp) << endl;
}
