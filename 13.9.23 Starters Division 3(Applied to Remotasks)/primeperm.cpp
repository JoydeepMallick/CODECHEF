...
void fun(ll start, ll end){
    for(ll i =start; i <= end; i+=4){
        cout << i-1 << " " << i << " " << i-3 << " " << i-2 << " ";
    }
}
...
void test(){
    ll n;
    cin >> n;
    /* for five "4 5 1 2 3 " given, for for six "4 5 6 1 2 3 " you can find this by hit and trial, for seven "3 4 5 6 7 1 2 " you can find this by hit and trial */

    /* if n%4==0 then it can be shown that for every consecutive four element i-1, i, i-3,i-2; will be correct combination where i is index of 4rth element of that combination because difference will be 2 which is prime because index of i-1 will be i-3 and index of i will be i-2 and index of i-3 will be i-1 and index of i-2 will be i. now if n%4==1 then (n-5)%4==0; if(n%4==2) then (n-6)%4==0; if(n%4==3) then (n-7)%4==0; */
