...
void test(){
  ...
  cin >> s;
  string news = s;
  /* just simple approach is that check for the first '1' from left to right and store its position in a variable pos now traverse from last and make all character '0' up to index pos and print s */

  //find highest index of 1 
  ll ind = 0;
  while(s[ind] != '1') ind++;
  if(ind < n-2 ){// only if not last two indices
    fori(ind+1, n){
      news[i] = '0';
    }
  }
