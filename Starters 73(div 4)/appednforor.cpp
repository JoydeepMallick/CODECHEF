...
void test(){
    ...
    //its obvious that the x can always be y but it may not be the minimum
/* we need to find which bits have been already been set by the array then the remaining bits which need to be set can be found out and thus the required minimum number */
/* hence xor of y and the y' where y' represents current or of array */
    
/* remember that we can turn on bits not off as its or operation hence if y turns out to be smaller than yprime then its an invalid situation as we can't turn off those already set bits */
    int ans = (yprime ^ y) ;
    if(yprime <= y && ans<=y ) cout << ans << endl;
    else cout << -1 << endl;
}
