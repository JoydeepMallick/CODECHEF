...
void test(){
    ll x,y;
    cin >> x >> y;
    //we are sure that x*y is always divisible by both x and y 
    //now n can  x*y-y which is divisible by y and x*y -x which is divisible by x
    //we need to equalize both
    //
    //a simple apporach would be to combine both i.e. n = x*y - x -y
    //hence x*y - x - y +x = y*(x-1) is divisible by y
    //similar concept goes for x also
    ll ans = x*y-x-y;
    //now we need to ensure that n is greater than equal to 1
    //so we can simply increase ans by a multiple of x*y as your choice to get ans as positive.
    while(ans < 1) ans+=x*y;
    cout << ans << endl;
}
