...
void test(){
    ...
    /*
     Here we notice that we need to minimize cost hence we need to minimize the number of values jumped to, hence we need to make minimum or operations and not make minimum jumps.

     Assume our answer will all bits on i.e. 1 1 1 1 1 .... 1
     now we need to check for each bit starting from MSB till LSB if thst particular bit is essential, if not turn it off so as to minimize the or answer.

     But how to implemnt it ???
