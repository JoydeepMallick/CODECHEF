...
void test() {
  ll n;
  cin >> n;
  // 3Aj + Ai - AiAj = 0 must satisfy
  // Ai = Aj only when Ai = 4, 0
  // else Ai = 6 and Aj = 2
  ...
  vll suff2(n, 0);
  suff2[n - 1] = (a[n - 1] == 2);
  rfor(i, n - 2, -1) { suff2[i] = suff2[i + 1] + (a[i] == 2); }
  dbg(suff2);

  fori(0, n) {
    if (a[i] == 6) {
      ans += suff2[i]; // for this 6 we have suff[2] number of 2s
    }
  }
