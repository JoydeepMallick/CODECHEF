...
template<typename T, typename V> // handles pair inside of iterator
...
void test(){
    //fix the mid element(s) and swap only remaining (ie sorted version of swappable elements only)
    int n, k;
    cin >> n >> k;
    int a[n+1];
    for( int i = 1; i <= n; i++) {
