...
void test(){
    ...
    int a[n][m], b[n][m];
    bool ok = true;//for row or column matrix no operation possible hence
                   //a must be equal to b
    fori(0,n) forj(0,m) cin >> a[i][j];
    fori(0,n) forj(0,m) {cin >> b[i][j]; if(a[i][j] != b[i][j]) ok = false;}
    /*
    Note that to perform flipping you always need square submatrices which is the operation given
    */
    if(n==1 || m==1){//for row or column matrices
        if(ok) yes;
        else no;
    }
    else{
        mii ea,oa,eb,ob;//odd and even index sum determines element to be
                        //placed  in repective maps for a and b
        /*
         *Observation :
         always the sum of coordinates of an element must have same parity(either both in a and b they are at even sum of indices or odd).
         If both have different parity such a position is invalid by the given set of operations.

         Here we simply create 2 maps for each depending on the parity type of elements coordinate sum. Then we check if such a situation is possible or not.
         */ 
        fori(0,n){
            forj(0,m){
                if((i+j) & 1) oa[a[i][j]]++;
        ...
        ok = true;
        for(auto apair : ea ){
            if(apair.second != eb[apair.first]){
                ok = false;
                break;
            }
        }
        if(ok){
            for(auto apair : oa ){
                if(apair.second != ob[apair.first]){
                    ok = false;
                    break;
                }
            }
