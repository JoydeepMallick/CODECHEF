...
void test(){
  ...
  mnheap pq;//default max priority_queue hence we need minimum priority_queue
  //[NOTE : in java by default min heap gets implemented]
  for(int ele : a) pq.push(ele);
  while(y){
      int minimum = pq.top();
      if((minimum^x) > minimum){
        pq.pop();//doesn't return any value unlike python
        pq.push(minimum^x);
        y--;//array updated one step done
      }
      else{
          if(y & 1) {//if y is odd
                pq.pop();
