...
void test(){
  int n;
  cin >> n;
  int p[n]={0},prefsum[n]={0},d;//no halt at last station
  //we are sure that train A is at least p[station 1 to 2] time ahead of B 
  //we can just find the difference of two elements of p sum then add -1 + p[first station halt]
  fori(1,n) {//1 based indexing
      cin >> p[i];
      prefsum[i]+=prefsum[i-1]+p[i];
  }
  fori(0,n-1){
    d = prefsum[i+1] - p[i];
    if(d > 0) p[i+1]+=p[i] + d;//halt of train B is required at current station by an amount d 
    else p[i+1]+=p[i];
  }
  cout << p[n-1] << endl;
}
