...
void test(){
    ...
    if(neg.size() == 0){
        int s = 0;
        s = accumulate(all(pos), s);
        cout << s << endl;
        return;
    }else if(pos.size() == 0){
        if(neg.size()%2 == 0){    
            int s2 = 0;
            s2 = accumulate(all(neg), s2);
            cout << - s2 << endl;
        }else{
            int s2 = 0;
            s2 = accumulate(all(neg), s2);
            int maxneg = MAX(neg);
            cout << - s2 + 2*maxneg << endl;
        }
    }
        ...
        int maxneg = MAX(neg);
        int minpos = MIN(pos);
        //replace smallest positive with maximum element in negative array only if abs value of neg > pos 
        int s1 = 0;
        s1 = accumulate(all(pos), s1); 
        int s2 = 0;
        s2 = accumulate(all(neg), s2);
        int ans;
