...
/* If the last three digits of a number are divisible by 8, then the number is completely divisible by 8 */
    ...
    //focus on last 3 digits only 
/* int num = stoi( (n.size() >=3) ? n.substr(n.size() - 3) : n); */
    string num =  (n.size() >=3) ? n.substr(n.size() - 3) : n;
    string remain = ((n.size() > 3) ? n.substr(0, n.size() - 3): "");
dbg(num, remain);
    if(stoi(num)%8 == 0) cout << n << endl;
    else{
      string ans;
      int mincnt = 4, actualnumsize = num.size();
     
      for(auto it : divby8){
        if(it.size() > actualnumsize){
            continue;
        }
        else if(it.size() < actualnumsize){
          int size = it.size();
          fori(0, actualnumsize - size){
            it = '0' + it;
          }
        }
        int cnt = 0;
        fori(0, actualnumsize){
          if(it[i] != num[i]) cnt++;
        }
        if(mincnt > cnt){
          mincnt = cnt;
