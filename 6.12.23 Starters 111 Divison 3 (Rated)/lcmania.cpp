...
void test(){
  ll n;
  cin >> n;
//powers of 2 not possible 
//
  // A B C    n  
  // 1 1 x    1 + 2x 
  // 1 x x    3x 
  // x x x    3x
  // if n is even then n = 2^m.y  where y is odd.  y = 2x + 1 
  // now the numbers are 2^m   2^m  2^m.x 
  // hence possible to generate all numbers 
  ll x;
  if(ceill(log2(n)) == floorl(log2(n))) cout << "-1\n";
  else if(n%2 == 1){
    x = (n-1)/2;
    cout << "1 1 " << x << endl;
  }
    ...
    x = (n-1)/2;
    ll powval = ( 1 << m);
    cout << powval << " " << powval << " " << powval*x << endl;
  }
