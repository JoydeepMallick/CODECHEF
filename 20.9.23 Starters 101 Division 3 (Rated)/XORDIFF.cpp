...
int result = 0; // Initialize the result to 0.
    int pos = -1; // Variable to keep track of the position of the rightmost different bit.
    bool flag = false; // A flag to indicate if there's a different bit.

    for (int i = 0; i < 30; i++) {
        int bitA = (1 << i) & a; // Get the i-th bit of 'a'.
        int bitB = (1 << i) & b; // Get the i-th bit of 'b'.

        if (bitA != bitB) {
            // If the bits are different, we need to change this bit in the result.
            // We set it to 1 in the result by using bitwise OR.
            if(bitA != (1 << i)) result = (result | (1 << i));

            // Update the position of the rightmost different bit.
            pos = i;

            // Set the flag to indicate that there's a different bit.
            flag = true;
        }
    }

    if (flag) {
        // If there was a different bit, we XOR the result with 1 to minimize it.
        result = (result ^ (1 << pos));
    }

    cout << result << endl; // Output the minimized result.
}
