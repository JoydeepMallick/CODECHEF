...
void test(){
    ...
    int b[n];
    read(b,n);
    //if j has 1 in a but 0 in b then not possible. 0 to 1 only possible.
    //edge case if all 0 in a then 1 cant be obtained.
    bool allzeros_a = true, allzeros_b = true, possible  = true;
    fori(0,n){
        if(a[i] == 1) {
            allzeros_a = false;
        }
