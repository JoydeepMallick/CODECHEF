...
void test(){
  ...
  //if no of odd elements odd not allowed 
//seperate odd and even numbers and sort them 
  vll odd, even;
  fori(0,n){
      if(b[i]&1)odd.pb(b[i]);
      else even.pb(b[i]);
  ...
  if(odd.size()&1){
    ...
    //pick ith and i + n/2 th number from each odd and even sorted elements to find ith and i + n/2th number 
    //
   
  sort(all(odd), greater<ll>()); sort(all(even), greater<ll>());
  dbg(odd, even);

  vll a(n, 0);
  int ind = 0, oddsize = odd.size(), evensize = even.size();

  fori(0,evensize/2){
    int x = (even[i] + even[i+evensize/2])/2;
    int y = even[i] - x;
    dbg(x,y);
    a[ind] = x;
    a[ind + n/2] = y;
  ...
  fori(0,oddsize/2){
    int x = (odd[i] + even[i+oddsize/2])/2;
    int y = odd[i] - x;
    dbg(x,y);
    a[ind] = x;
    a[ind + n/2] = y;
