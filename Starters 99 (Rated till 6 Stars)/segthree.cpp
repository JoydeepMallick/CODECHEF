...
void test(){
    ...
    /*
     Here we will use sliding window for consequitive 3 elements and update the the last element in the window.


Note :- (+ mean updation done)

_ _ + + + + + +......   

Note that first two elemtns never get updates so we might not always get the minimum operations with this move.


So we will try for possible combinations of first and 2nd elemt so that we can find the minimum and them update accordingly


i.e.   [1st element + (0,1,2)] X [2nd element + (0,1,2)]  and then perfrom sliding window and minmum of all possible combinations(total 9 combinations) is our final answer. 
     */
    fori(0,3){
        forj(0,3){
            ll a1 = a[0];
            ...
            fork(2,n){
                ...
                while (total%3 != 0) {
                    a3++;
                    total++;//so that it can reach exit condition 
                    increment++;
                }
                //update a1 and a2 in new window 
                a1 = a2;
                a2 = a3;
            }
