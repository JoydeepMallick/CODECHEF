...
void test(){
    ...
    vector<pair<char, int>> freq = {{s[0],1}};
    int curind = 0;
    fori(1,n){
      if(s[i] == s[i-1]) freq[curind].second++;
      else{
...
/* dbg(freq); */
    string news = "";
    for(auto it : freq){
        if(it.second < 3) while(it.second--) news += it.first;
        else{
          if(it.second&1) news+=it.first;
          else fori(0,2) news += it.first;
        }
    }
    cout << news << endl;
}
