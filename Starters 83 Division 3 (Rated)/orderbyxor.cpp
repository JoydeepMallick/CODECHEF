...
/* int bitcount = log2(a) + 1; */
/* ull ans = (1 << bitcount);//max element's msb value i.e. if 9 then log = 3 i.e. ans = 1 << (msb+1) = 16(i.e a bit higher) */
/* cout <<  ans-1 << endl;// ie highest value possible for max value msb as limit i.e.10000 -1 = 1111 if msb = 3 (3 2 1 0) */
        rfor(i, 30, -1, -1){
            int x = (1 << i) ^ a;
            int y = (1 << i) ^ b;
            int y = (1 << i) ^ c;
