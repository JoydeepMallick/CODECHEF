...
void test(){
  ...
  bitset<30> N = bitset<30>(n);

/* cout << "bitset : " << N << endl; */

  ll first0fromleft = -1;
  bool first1 = false, sec0 = false, third1 = false;
  rfor(i,29, -1){
    if(first1 == false){
      if(N[i] == 0) continue;
      first1 = true; // first 1 encountered
    }
    else if(sec0 == false){
      if(N[i] == 1) continue;
      sec0 = true;
  ...
  dbg(first1, sec0, third1);
  if(first1 && sec0 && third1){//101 subsequence exists
    ll ans = 0;
    //store index of first set bit from right 
    ll first1fromright = -1;
    fori(0, first0fromleft){
      if(N[i] == 1){
        first1fromright = i;
        break;
      }
    }
    ans += (1 << first1fromright);
   
    rfor(i,first0fromleft-1, first1fromright){
      if(N[i] == 0){
        ans += (1 << (i));
      }
    }
